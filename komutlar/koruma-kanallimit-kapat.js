...
//return message.channel.send('(Kurucu Olda Gel Aslan)Yeterli Yetkiye Sahip Görünmüyorsun! '+client.emojis.get('647760202875142154'))
  if (!db.fetch(`klimit31_${message.guild.id}`)) return message.channel.send(
new Discord.RichEmbed()
...
.setColor('BLACK')
.setDescription('Koruma Kanal Sistemi Sunucunuz İçin Kapatıldı.')
.setTimestamp()
.setFooter(`${client.user.username}`, client.user.avatarURL)
)
