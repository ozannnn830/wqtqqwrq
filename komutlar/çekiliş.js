...
exports.run = async (client, message, args) => {
  if (!args[0]) return message.channel.send(`**Doğru Kullanım** \`w!çekiliş <#kanal> <süre> <saniye/dakika/saat/gün> <ödül>\``)
  message.delete(4000)
  
    if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(`Bu komutu kullanabilmek için "\`Üyeleri Yasaklama\`" iznine ihtiyacın var.`).then(msg => msg.delete(5000));
  ...
  .setDescription(`**Çekiliş kanalını etiketlemelisin !** ***\`\`\`${prefix}çekiliş #çekiliş 1 gün 20TL\`\`\`\n***`)
      .setTimestamp()
...

  {
    ...
    .setColor("ff0000")
    .setTitle(`${message.guild.name} - Çekiliş Sistemi `)
    .setDescription(`**Bir süre yazmalısın !** ***\`\`\`${prefix}çekiliş #çekiliş 1 gün 20TL\`\`\`\n***`)
        .setTimestamp()
...

  {
    ...
    .setColor("ff0000")
    .setTitle(`${message.guild.name} - Çekiliş Sistemi `)
     .setDescription(`**Süreyi doğru yazmalısın !** ***\`\`\`${prefix}çekiliş #çekiliş 1 gün 20TL\`\`\`\n***`)
        .setTimestamp()
  ...
  
  {
    ...
    .setColor("ff0000")
    .setTitle(`${message.guild.name} - Çekiliş Sistemi `)
    .setDescription(`**Ödülü yazmalısın.** ***\`\`\`${prefix}çekiliş #çekiliş 1 gün 20TL\`\`\`\n***`)
        .setTimestamp()
  ...
  .setDescription(`:tada:  emojisine tıklayarak çekilişe katılabilirsiniz.\n\n**Çekiliş Süresi : **\` ${duration} ${sure}\``)
  .setTimestamp()
  ...
  room.send(giveEmbed).then(m => {
    ...
    setTimeout(() => {
      ...
      if (gFilter == client.user) gFilter = `${gFilter}`
